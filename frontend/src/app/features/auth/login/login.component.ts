          ...
          @if (googleEnabled()) {
            <div class="relative my-6">
              ...
              <div class="relative flex justify-center text-sm">
                <span class="px-3 bg-surface-50 text-surface-500">{{ 'auth.orContinueWith' | t }}</span>
              </div>
            </div>
            ...
            <button 
              ...
              class="w-full flex items-center justify-center gap-3 py-3 px-4 border border-surface-300 rounded-xl bg-white text-surface-700 font-medium hover:bg-surface-50 transition-colors"
            >
...
export class LoginComponent {
  ...
  checkGoogleConfig() {
    this.authService.getGoogleConfig().subscribe({
      next: (res) => this.googleEnabled.set(res.enabled),
