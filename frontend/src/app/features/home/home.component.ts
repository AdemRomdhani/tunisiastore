...
@Component({
  ...
  template: `
    <div class="min-h-screen">
      ...
      <section class="relative bg-gradient-to-br from-primary-600 via-primary-700 to-primary-900 text-white overflow-hidden">
        ...
        <div class="container mx-auto px-4 py-20 lg:py-32 relative">
          <div class="grid lg:grid-cols-2 gap-12 items-center">
            <div class="max-w-2xl">
              <span class="inline-flex items-center gap-2 bg-white/10 text-white text-sm font-medium px-4 py-1.5 rounded-full mb-6">
              ...
              <div class="flex flex-wrap gap-4">
                <a routerLink="/products" class="group bg-white text-primary-600 px-8 py-4 rounded-xl font-semibold hover:bg-gray-100 transition shadow-lg hover:shadow-xl">
                  Découvrir
                  <svg class="inline-block ml-2 w-5 h-5 group-hover:translate-x-1 transition" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8l4 4m0 0l-4 4m4-4H3"/>
                  </svg>
                </a>
                <a routerLink="/products" [queryParams]="{onSale: true}" class="flex items-center gap-2 border-2 border-white/30 text-white px-8 py-4 rounded-xl font-semibold hover:bg-white/10 transition">
        ...
        <div class="container mx-auto px-4">
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            @for (feature of features; track feature.title) {
              <div class="group flex items-center gap-4 p-6 rounded-2xl hover:bg-primary-50 transition-all duration-300">
                <div class="w-14 h-14 bg-primary-100 rounded-xl flex items-center justify-center text-primary-600 group-hover:scale-110 transition-transform">
          ...
          <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
            @for (cat of categories; track cat.name) {
              <a [routerLink]="['/products']" [queryParams]="{category: cat.slug}" 
                 class="group bg-white p-6 rounded-2xl text-center hover:shadow-lg transition border border-gray-100">
                <div class="text-4xl mb-3 group-hover:scale-125 transition">{{ cat.icon }}</div>
                <div class="font-medium text-gray-900 group-hover:text-primary-600 transition">{{ cat.name }}</div>
              </a>
            }
          </div>
        </div>
        ...
        <div class="container mx-auto px-4">
          <div class="flex items-center justify-between mb-8">
            <div>
              ...
              <p class="text-gray-500 mt-1">Les meilleures selections de nos clients</p>
            </div>
            <a routerLink="/products" class="text-primary-600 font-medium hover:underline flex items-center gap-1">
              Voir tout
      ...
      <section class="py-8">
        <div class="container mx-auto px-4">
          <a routerLink="/products" [queryParams]="{onSale: true}" class="block relative rounded-3xl overflow-hidden group">
            <div class="bg-gradient-to-r from-red-600 to-red-700 px-8 py-12 flex items-center justify-between">
              <div>
                <span class="inline-block bg-white/20 text-white text-sm font-medium px-3 py-1 rounded-full mb-4">OFFRE LIMITÉE</span>
                <h2 class="text-3xl lg:text-4xl font-bold text-white mb-2">Hasta -50%</h2>
                <p class="text-red-100">Sur une sélection de produits</p>
              </div>
              <div class="hidden lg:block">
                <div class="text-6xl font-bold text-white/20">SALE</div>
              </div>
            </div>
            <div class="absolute right-8 top-1/2 -translate-y-1/2 w-16 h-16 bg-white rounded-full flex items-center justify-center group-hover:translate-x-2 transition">
      ...
      <!-- Flash Deals Section -->
      <section class="py-12 bg-gradient-to-r from-orange-500 to-red-600">
        <div class="container mx-auto px-4">
          <div class="flex items-center justify-between mb-8">
            <div class="flex items-center gap-4">
              <div class="bg-white text-orange-600 px-6 py-3 rounded-2xl">
              ...
              <div>
                <h2 class="text-2xl font-bold text-white">Offres Flash</h2>
                <p class="text-orange-100">Ne manquez pas ces affaires limitées!</p>
              </div>
            </div>
            <div class="flex items-center gap-2 bg-white/10 px-4 py-2 rounded-full">
              <span class="text-white text-sm font-medium">Fin dans:</span>
              @if (flashDeals().length > 0 && flashDeals()[0].saleEndsAt) {
                <div class="flex items-center gap-1">
                  <span class="bg-white text-orange-600 font-bold px-3 py-1 rounded-lg">{{ getProductTimer(flashDeals()[0]._id).days }}j</span>
                  <span class="text-white">:</span>
                  <span class="bg-white text-orange-600 font-bold px-3 py-1 rounded-lg">{{ getProductTimer(flashDeals()[0]._id).hours }}h</span>
                  <span class="text-white">:</span>
                  <span class="bg-white text-orange-600 font-bold px-3 py-1 rounded-lg">{{ getProductTimer(flashDeals()[0]._id).minutes }}m</span>
                  <span class="text-white">:</span>
                  <span class="bg-white text-orange-600 font-bold px-3 py-1 rounded-lg">{{ getProductTimer(flashDeals()[0]._id).seconds }}s</span>
                </div>
              }
            </div>
          </div>
          ...
          @if (flashDeals().length > 0) {
            ...
            <div class="text-center py-8">
              <p class="text-white/70 text-lg">Revenez bientôt pour les prochaines offres flash!</p>
            </div>
          }

          <div class="text-center mt-8">
            <a routerLink="/products" [queryParams]="{onSale: true}" class="inline-flex items-center gap-2 bg-white text-orange-600 px-8 py-3 rounded-xl font-semibold hover:bg-gray-100 transition shadow-lg">
        ...
        <div class="container mx-auto px-4">
          <div class="flex items-center justify-between mb-8">
            <div>
              ...
              <p class="text-gray-500 mt-1">Les derniers ajouts à notre collection</p>
            </div>
            <a routerLink="/products" [queryParams]="{sort: '-createdAt'}" class="text-primary-600 font-medium hover:underline flex items-center gap-1">
              Voir tout
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
              </svg>
      ...
      <section class="py-16 bg-gradient-to-r from-indigo-600 to-purple-600">
        <div class="container mx-auto px-4">
          <div class="flex items-center justify-between mb-8">
            <div>
              <h2 class="text-2xl font-bold text-white">Packs & Bundles</h2>
              <p class="text-indigo-100 mt-1">Économisez plus avec nos packages</p>
            </div>
            <a routerLink="/bundles" class="bg-white text-indigo-600 px-4 py-2 rounded-lg font-medium hover:bg-indigo-50 transition">
              Voir tout
            </a>
          </div>
      ...
      <section class="py-16 bg-white">
        <div class="container mx-auto px-4">
          <div class="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              <h2 class="text-3xl font-bold text-gray-900 mb-6">Pourquoi choisir Tunisia Store?</h2>
              <div class="space-y-6">
                @for (item of whyUs; track item.title) {
                  <div class="flex gap-4">
                    <div class="w-10 h-10 bg-primary-100 rounded-xl flex items-center justify-center text-primary-600 flex-shrink-0">
                      <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="relative">
              <div class="bg-gradient-to-br from-primary-500 to-primary-700 rounded-3xl p-8 text-white">
                <div class="text-center">
                  <div class="text-6xl font-bold mb-2">4.9</div>
                  ...
                  <div class="text-lg font-medium">Note moyenne</div>
                  <div class="text-primary-200">Basé sur 500+ avis</div>
                </div>
              </div>
            </div>
          </div>
      ...
      <section class="py-16 bg-gray-900">
        <div class="container mx-auto px-4">
          <div class="bg-gradient-to-r from-primary-600 to-primary-700 rounded-3xl p-8 lg:p-12 text-center text-white flex flex-col items-center">
            ...
            <h2 class="text-3xl lg:text-4xl font-bold mb-4">Besoin d'aide ?</h2>
            ...
            <a routerLink="/contact" class="inline-flex items-center gap-2 bg-white text-primary-600 px-8 py-4 rounded-xl font-bold hover:bg-gray-100 transition shadow-lg hover:shadow-xl transform hover:-translate-y-1">
              Nous contacter
              <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
