    ...
    <div class="container mx-auto px-4 py-16 text-center min-h-screen">
      @if (loading()) {
        <div class="max-w-md mx-auto">
          ...
          <h2 class="text-2xl font-bold mb-2">Traitement du paiement...</h2>
          <p class="text-gray-500">Veuillez patienter, nous préparons votre transaction sécurisée.</p>
        </div>
        ...
        <div class="max-w-md mx-auto bg-green-50 border border-green-200 rounded-xl p-8">
          ...
          <h2 class="text-2xl font-bold text-green-700 mb-2">Paiement réussi!</h2>
          <p class="text-gray-600 mb-2">Votre commande a été confirmée.</p>
          <p class="text-sm text-gray-500 mb-6">Numéro de commande: {{ orderNumber() }}</p>
          <a routerLink="/orders" class="btn-primary inline-block">Voir mes commandes</a>
        </div>
        ...
        <div class="max-w-md mx-auto bg-red-50 border border-red-200 rounded-xl p-8">
          ...
          <h2 class="text-2xl font-bold text-red-700 mb-2">Paiement échoué</h2>
          <p class="text-gray-600 mb-6">{{ error() }}</p>
          <a routerLink="/checkout" class="btn-primary inline-block">Réessayer</a>
        </div>
        ...
        <div class="max-w-md mx-auto bg-yellow-50 border border-yellow-200 rounded-xl p-8">
          ...
          <h2 class="text-2xl font-bold text-yellow-700 mb-2">Paiement annulé</h2>
          <p class="text-gray-600 mb-6">Vous avez annulé le paiement.</p>
          <a routerLink="/checkout" class="btn-primary inline-block">Retour au checkout</a>
        </div>
      } @else if (error()) {
        <div class="max-w-md mx-auto bg-red-50 border border-red-200 rounded-xl p-8">
          <div class="w-16 h-16 bg-red-100 rounded-full flex items-center justify-center mx-auto mb-4">
        ...
        <div class="max-w-md mx-auto">
          <h2 class="text-2xl font-bold mb-4">Redirection vers la banque...</h2>
          <p class="text-gray-500">Si vous n'êtes pas redirigé automatiquement, cliquez ci-dessous.</p>
        </div>
      }
    </div>
  `
